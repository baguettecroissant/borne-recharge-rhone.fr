/**
 * Geographic proximity linking engine for the Rhône (69).
...
export function getNearbyCommunes(
  ...
  return allCommunes
    ...
    .map(c => {
      ...
      // Approximate physical distance in km using local projection (45.75°N for Rhône)
      // 1 degree of Latitude ≈ 111.1 km
      // 1 degree of Longitude ≈ 111.1 * cos(45.75°) ≈ 77.5 km
      const dLat = (lat - currentLat) * 111.1;
      const dLon = (lon - currentLon) * 77.5;
